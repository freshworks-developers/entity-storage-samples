...
exports = {
  ...
  onTicketCreateHandler: function (args) {
    ...
    if (args.data.ticket.tags && args.data.ticket.tags.includes("delivery_request")) {
      ...
      Promise.resolve((util.parseStandardFields(args.data)))
        ...
        .then((response) => {
          util.logIt(args.iparams["logger-url"], "entityRecordCreated", response.data);
          console.info("(4/4) Entity record creation was successful!");
        })
        .catch((err) => {
          console.error("Something went wrong", err);
          util.logIt(args.iparams["logger-url"], "entityRecordCreateFailed", err);
        });
