...
function registerEvents(map) {
    ...
    map.on('mouseenter', 'delivery_request_circle', function (e) {
        ...
        var custom_html =
            ...
            <img width="100px" class="align-self-center ml-2 mr-3" src="https://avatar.oxro.io/avatar.svg?name=${prop.contact_name}.svg&fontSize=120&bold=true" alt="">
            <div class="media-body">
              <h5 class="mt-0">  ${prop.contact_name}  </h5>
              <p>📍 ${prop.loc_locality}, ${prop.loc_area2}, ${prop.loc_area1}<br/>
                 🛒 ${prop.category == "[M]" ? "Dairy Products" : "Fresh Produce"} <br/>
                <fw-label value='$ ${prop.cart_value}' color='green'> </fw-label> </p>
            </div>
          </div>
            `;
    ...
    map.on('mousemove', function (e) {
        if (planningMode) {
            ...
            map.getSource("planning-ds").setData(createGeoJSONCircle([e.lngLat.lng, e.lngLat.lat], nCov));
            planCoverageAreas = [];
            planCoverageAreas = pointsOnScreen.filter((i) => {
                if (distance(i.geometry.coordinates[1], i.geometry.coordinates[0], parseFloat(e.lngLat.lat), parseFloat(e.lngLat.lng)) < nCov) {
                    return true;
                }
            });
    ...
    map.on('click', 'delivery_request_circle', function (e) {
        ...
        map.flyTo({
            zoom: 20,
            center: [
                d_app.active_item.loc_long,
                d_app.active_item.loc_lat
            ],
...
function updateMarkers(map) {
    ...
    document.getElementById("tile-y-number").innerHTML = "<small>$</small> " + total;
}

var createGeoJSONCircle = function (center, radiusInKm, points, prop = {
    description: ""
}) {
    if (!points) points = 64;
